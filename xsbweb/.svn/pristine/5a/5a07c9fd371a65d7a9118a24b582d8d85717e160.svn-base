                	...
                	<c:if test="${trsProjectVOList!=null}">
	                	<c:forEach items="${trsProjectVOList}" var="project" varStatus="index">
	                		<tr>
	                			...
	                			<td id="project_${index.index}" style="hidden">${project.projectNo}</td>
	                			<td>
	                				<c:if test="${project.projCpCode > '40000' }">
	                					<a href="http://<%=hostName%>:8080/road/detail/roadItems.do?IsRoadOrWbeview=1&projectNo=${project.projectNo}&objectType=project" target="_blank">${project.projCpCode}</a>
	                				</c:if>
	                				<c:if test="${project.projCpCode < '40000' }">
	                					${project.projCpCode}
	                				</c:if>
		                       ...
		                       <td>
		                        ...
		                        <td>	
		                        	...
		                        	<%-- <c:if test="${project.projCpCode > '40000' }">
		                        		| <a href="http://<%=hostName%>:8080/road/detail/roadItems.do?IsRoadOrWbeview=1&projectNo=${project.projectNo}&objectType=project" target="_blank">预览</a>
		                        	</c:if> --%>
		                    </tr>
	                	</c:forEach>
                	</c:if>
...
<!--</form>-->
 ...
 <script type="text/javascript" language="javascript">
	...
	function resetOrder(){
		...
		$.ajax({
  			url : "admin/project/resetOrder.do?projRole=16777216",
			async: false,
			success : function(data) {
				if (data.resultCode == '1') {
					window.location.reload();
