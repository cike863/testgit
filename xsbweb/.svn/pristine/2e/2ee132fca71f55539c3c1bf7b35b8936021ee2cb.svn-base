...
public class AppTrsProjectController {
	...
	public @ResponseBody Map<String,Object> toRoadShowIndex(
		...
		try {
			...
			if(xsbTotalVO2!=null){
				...
				xsbTotalVO.setCustomerCounts(xsbTotalVO2.getCustomerCounts());
			}
		...
		try {
			...
			if(counts>0 && counts!=3){
				...
				if(trsProject!=null){
					...
					String content3 = "恭喜您报名["+trsProject.getProjName()+"]成功。【第一路演】";
					String resultStr3 = SmsUtil.sendPhoneSms(customerPhoneNo, content3.toString());
					if(!"success".equals(resultStr3)){
						log.info("##############发送用户提示报名成功短信失败！");
					}
				}
			}else{
				map.put("isApply", "0");
		...
		try {
			...
			if(registerFlag>0){
				...
				if(trsProject!=null){
					...
					.append("。【第一路演】");
					//String resultStr2 = SmsUtil.sendPhoneSms("18124198820", content2.toString());
					log.info(trsProject.getStaffInfo().get("staffPhone"));
					String resultStr2 = SmsUtil.sendPhoneSms(trsProject.getStaffInfo().get("staffPhone"), content2.toString());
					if(!"success".equals(resultStr2)){
						log.info("##############发送项目经理短信失败！");
					}
	...
	@RequestMapping(value="/app/project/getProjectPvByNo",method=RequestMethod.GET)
	@ResponseBody
	public Map<String,Object> getProjectPvByProjectNo(
			@RequestParam("projectNo")String projectNo,
			@RequestParam("loginMethod") String loginMethod){
		Map<String,Object> map = new HashMap<String,Object>();
		//参数错误
		...
		int projectPv = 0;
		try{
			projectPv = trsProjectService.getProjectPvByProjectNo(projectNo);
		}catch(Exception e){
			e.printStackTrace();
			projectPv = 0;
		}
		map.put("projectPv", projectPv);
		return map;
	}
}
