...
public class CustomerServiceImpl implements CustomerService {
	...
	public CustomerVO getCustomerById(String customerId,List<EnumVO> gzIndustryEnumList,List<EnumVO> capitalScaleEnumList,List<EnumVO> customerTypeEnumList) throws Exception {
		...
		customerList.get(0).setCustomerCases(ExcelUtil.toGzIndustrys(customerList.get(0).getGzIndustry()+"",gzIndustryEnumList));
		customerList.get(0).setCapitalScale(ExcelUtil.toCapitalScale(customerList.get(0).getCapitalScale(),capitalScaleEnumList));
		customerList.get(0).setCustomerType(ExcelUtil.toCustomerType(customerList.get(0).getCustomerType(),customerTypeEnumList));
	...
	public List<Customer> getCustomerByProjectNo(String projectNo) throws Exception {
		...
		customerVO.setObjectNo(projectNo);
		//customerVO.setSignStatus("1");//用来判断是项目报名导出还是用户列表导出1为项目报名导出
