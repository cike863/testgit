...
// 			alert($("span").length);
  ...
  <body>
	...
	<div class="container-full">
		<div class="row">
			<div class="col-md-12 mt10">
				<form:form action="admin/item/add"  name="newsForm" method="post" class="form-horizontal">
					<div class="form-group">
						<label class="col-md-1 control-label">名称</label>
						<div class="col-md-7">
							<input type="text" class="form-control" name="itemKey" value="${item.itemKey}"placeholder="名称"/><form:errors path="itemKey"></form:errors>
							<input type="hidden" name="objectNo" value="${item.objectNo}"/>
							<c:if test="${item.oldItemKey ne null}">
								<input type="hidden" name="oldItemKey" value="${item.oldItemKey}"/>
							</c:if>
						</div>
					</div>
					<div class="form-group">
						<label class="col-md-1 control-label">位置顺序</label>
						<div class="col-md-7">
							<input type="text" class="form-control" name="itemLocationOrder" value="${item.itemLocationOrder eq null ? 0 : item.itemLocationOrder}" placeholder="位置顺序"/><form:errors path="itemLocationOrder"></form:errors>
						</div>
					</div>
					<div class="form-group">
						<label class="col-md-1 control-label">资源</label>
						<div class="col-md-7">
							<input type="hidden" class="form-control" name="itemMediaNo" value="${item.itemMediaNo }" placeholder="位置顺序"/><form:errors path="itemMediaNo"></form:errors>
							<img id="itemMediaPic" src="" height="100" width="100">
							<input type="button" onclick="toUpload('itemMediaNo','image');" value="上传" class="btn btn-success"/>
                     		<input type="button" onclick="toChear('itemMediaPic','itemMediaNo');" value="清空" class="btn"/>
					...
					<div class="form-group">
						<label class="col-md-1 control-label">内容</label>
						<div class="col-md-9">
							<textarea name="itemValue" style="width: 80% !important;height: 500px !important;" cols="100" rows="8">${item.itemValue}</textarea>
						</div>
					</div>
					<div class="row">
						<div class="col-md-9 mt10 text-center">
