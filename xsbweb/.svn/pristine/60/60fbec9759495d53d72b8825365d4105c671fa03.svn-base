	...
	public TrsProject getProjectByNo(String projectNo) throws Exception {
		...
		if(trsProject!=null && CommonUtils.isNotBlank(trsProject.getProjectNo())){
			...
			if(trsProject.getProjPercentage()!=null){
				if(trsProject.getProjPercentage()>1){
					trsProject.setProjPercentage(1d);
				}else{
			...
			if(CommonUtils.isNotBlank(trsProject.getHandler())){
				...
				if(staff!=null){
					...
					trsProject.getStaffInfo().put("staffWebchat", CommonUtils.isBlank(staff.getStaffWebchat())?"":staff.getStaffWebchat());
					trsProject.getStaffInfo().put("staffPicPath", CommonUtils.isBlank(staff.getStaffPicPath())?"":staff.getStaffPicPath());
					trsProject.getStaffInfo().put("staffPosition", CommonUtils.isBlank(staff.getStaffPosition())?"":staff.getStaffPosition());
				}
			}
			//获取projectItem
