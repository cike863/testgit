	...
	public @ResponseBody Map<String,Object> getViodeMeetByMeetNo(
		...
		try {
			...
			if(meetVideoRoomVO!=null){
				String timeof = "";
				if(CommonUtils.isNotBlank(meetVideoRoomVO.getStartMinute())){
					timeof = CommonUtils.dateOfTwo(new Date(), new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(meetVideoRoomVO.getStartMinute()));
				}else{
					timeof = "预告";
				}
				meetVideoRoomVO.setStartMinute(timeof);
				//视频role处理，便于前台数据处理
				String type = "1";
