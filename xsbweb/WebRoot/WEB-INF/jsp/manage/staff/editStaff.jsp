...
<html>
	...
	<div class="container-full">
		<div class="row">
			<div class="col-md-12 mt10">
				<form:form action="admin/staff/editStaff.do"  name="staffForm" method="post" class="form-horizontal">
				   <input type="hidden" name="staffId" value="${staff.staffId }"/>
	                ...
	                <div class="form-group">
						...
						<div class="col-md-7"><!-- value="${staff.staffPassword }" -->
							<input type="text" class="form-control" name="staffPassword"  placeholder="请设置密码"/><form:errors path="staffPassword"></form:errors>
						</div>
					</div>
					<div class="form-group">
  ...
  window.onload = function(){
		 if(${staff.departmentId !=''}){
			var select = $("#departmentId option");				 
			for(var i=0; i<select.length; i++){
		        if(select[i].value == '${staff.departmentId}'){
  ...
  function toUpload(mediaType){
		...
		function uploadIsOk(mediaType,mediaNo,url,returnPath,newFileName,format,size){
				$("[name=staffPicPath]").val(returnPath);
				$("#staffPic").attr("src",returnPath);
		}
