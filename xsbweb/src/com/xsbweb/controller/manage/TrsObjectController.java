...
public class TrsObjectController {
	...
	public ModelAndView toProjectList(TrsProject trsProject,HttpServletRequest request) throws Exception{
		ModelAndView mav = new ModelAndView();
		MultipleDataSource.setDataSourceKey(DataBaseConstant.OLTP);
		//项目中，项目部以及市场人员只看自己项目过滤
			...
			if(staffList!=null && !staffList.isEmpty()){
				for(Staff staffs : staffList){
					if(staffs!=null&&staffs.getStaffId().equals(customerId)){
						trsProject.setHandler(customerId);
						break;
					}
				}
			}
...
/*	//视频直播管理,列表用图片,上传显示的数据;
	...
	public ModelAndView queryApplyList(Customer customer)throws Exception{
		...
		for(Customer customer1 : custList){
			customer1.setCustomerInstttCode(ExcelUtil.toGzIndustrys(customer1.getGzIndustry()+"",gzIndustryEnumList));//关注行业用customerInstttCode代替
			customer1.setCapitalScale(ExcelUtil.toCapitalScale(customer1.getCapitalScale(),capitalScaleEnumList));
			customer1.setCustomerType(ExcelUtil.toCustomerType(customer1.getCustomerType(),customerTypeEnumList));
		}
	...
	public Map<String,Object> getExcel(@RequestParam("theFilePath") String filepath,@RequestParam("projectNo")String projectNo,HttpServletResponse response,HttpServletRequest request) throws Exception{
		...
		List<EnumVO> customerTypeEnumList = xsbBaseService.getEnumVOListByColumnFullName(XsbBusinessConstant.CUSTOMER_TYPE);
		try{
			customerList = ExcelUtil.getAllByExcel(filepath,gzIndustryEnumList,capitalScaleEnumList,customerTypeEnumList);
			int result=0;
	...
	public Map<String,Object> toExcel(@RequestParam("projectNo")String projectNo,HttpServletRequest request,HttpServletResponse response) throws Exception{
		...
		List<EnumVO> customerTypeEnumList = xsbBaseService.getEnumVOListByColumnFullName(XsbBusinessConstant.CUSTOMER_TYPE);
		try{
			ExcelUtil.toExcel(customerList,response,XsbBusinessConstant.OBJECT_EXCEL,gzIndustryEnumList,capitalScaleEnumList,customerTypeEnumList);
			map.put(ResultCode.RESULT_CODE, ResultCode.RESPONSE_SUCCESS);
		}catch(Exception e){
			map.put(ResultCode.RESULT_CODE, ResultCode.RESPONSE_FAIL);
		} 
	...
	public ModelAndView addItems(ProjectItem item) throws Exception{
		...
		if(!item.getItemKey().equals(item.getOldItemKey())){//更新时key值没有更新过滤，新增时oldItemKey为对象null而ItemKey为string类型，默认""
			List<ProjectItem> items = trsProjectService.getProjectItemListByNo(item);
			if (!items.isEmpty() && items.get(0) != null) {
				mav.addObject("error", "存在相同的key值");
		...
		if (result > 0) {
			...
			item.setItemValue(item.getItemValue().replaceAll("/dyly/",(XsbBusinessConstant.DYLY_URL+"/")));
	...
	@RequestMapping(value="/project/{projectNo}/{customerIdArrs}/del")
	@ResponseBody
	public Map<String,Integer> batchDeleteApplyCustomer(
			@PathVariable("projectNo") String  projectNo,
			@PathVariable("customerIdArrs") String[] customerIdArrs){
		Map<String,Integer> map = new HashMap<String,Integer>();
		try{
			MultipleDataSource.setDataSourceKey(DataBaseConstant.OLTP);
			int result = trsProjectService.batchDeleteApplyCustomer(projectNo,customerIdArrs);
			if(result>0){
				map.put(ResultCode.RESULT_CODE, ResultCode.RESPONSE_SUCCESS);
			}else{
				map.put(ResultCode.RESULT_CODE, ResultCode.RESPONSE_FAIL);
