...
public class EnumController {
   ...
   public ModelAndView toAddStaff(EnumVO enumVO)throws Exception{
    ...
    public ModelAndView getScrollListByObjectId(@RequestParam("objectId")String objectId) throws Exception{
    	...
    	EnumVO enumVO = new EnumVO();
    	enumVO.setEnumDescCn("广告");
    	List<EnumVO> adScrollList = enumService.getScrollList(enumVO);
    	Set<String> adEnumDesc = new HashSet<String>();
    	for(EnumVO enumVO1 : adScrollList){
    		adEnumDesc.add(enumVO1.getEnumDescCn());
    	}
    	if(objectId.contains("news")){
    		enumVO.setEnumDescCn("轮播");
    		List<EnumVO> lbScrollList = enumService.getScrollList(enumVO);
    		Set<String> lbEnumDesc = new HashSet<String>();
        	for(EnumVO enumVO1 : lbScrollList){
        		lbEnumDesc.add(enumVO1.getEnumDescCn());
        	}
        	mav.addObject("lbEnumDesc",lbEnumDesc);
    	}
    	mav.addObject("scrollList",scrollList);
    	mav.addObject("checkEnumDesc",checkEnumDesc);
    	mav.addObject("adEnumDesc",adEnumDesc);
