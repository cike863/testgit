...
public class PagerTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		sb.append(".pagination {padding: 5px;float:left;font-size:12px;width:1000px;}");
		...
		if(recordCount == 0){
			...
			//每页显示条数
			sb.append("每页条数");
			sb.append("<select id=\"pageSizeSelect\" name=\"pageSize\">\r\n");
			for(int i = 1; i <= 6; i++){
				if(this.pageSize == i*5){
					sb.append("<option  value="+i*5+" selected=\"selected\">"+i*5+"</option>\r\n");
				}else{
					sb.append("<option  value="+i*5+">"+i*5+"</option>\r\n");
				}
			}
			sb.append("</select>\r\n");
			sb.append("</form>\r\n");
			...
			// 生成提交表单的JS
			//sb.append("<script type=\"text/javascript\" language=\"javascript\"  src="+request.getContextPath()+"/js/util/jquery-1.11.3.js\"></script>");
			...
			sb.append("$('#pageSizeSelect').change(function(){ \r\n");
			sb.append("var pageSize = $(this).children('option:selected').val();\r\n");
			sb.append("$('input[name=pageSize]').val(pageSize)\r\n");
			sb.append("});\r\n");
			sb.append("};\r\n");
			sb.append("</script>\r\n");
		}
